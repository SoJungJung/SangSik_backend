...
app.use(
  cors({
    origin: "https://web-sangsikquiz-m2l7w1ydc2132f7e.sel4.cloudtype.app", // 프론트엔드 URL로 제한
  })
);
...
app.post("/api/submit-score", async (req, res) => {
  const { device_id, score, nickname } = req.body;
  const ip_address = req.body.ip_address || req.ip; // 기본적으로 클라이언트 IP를 가져옴

  try {
    // 기존 사용자 확인
    console.log("점수 제출 요청 데이터:", req.body);
    const existingUser = await pool.query("SELECT * FROM ranking WHERE device_id = $1 ORDER BY datetime DESC LIMIT 1", [
      device_id,
    ]);
